...
RUN mkdir /tmp/nginx
RUN tar zxf /tmp/nginx.tar.gz -C /tmp/nginx --strip-components 1
...
RUN fpm \
    ...
    -t deb \
    -m "${maintainer}" \
    --description "nginx Nginx build by Will Jessop. See Homepage metadata for build info link." \
    --url "https://github.com/wjessop/nginx_build" \
    -n nginx \
    --iteration $iteration \
    -a amd64 \
    -v $version \
